...
Sections 2–4 define the governance model and protocol invariants. Sections 5–7 specify the execution pipeline. Sections 8–10 describe the orchestration architecture and on-chain integration. Section 11 presents empirical validation against disputed markets. Section 12 surveys related work. Sections 13–16 cover the threat model, design rationale, and future directions. Appendices A–D provide settlement pseudocode, a glossary, CRE workflow definition, and a formal termination argument.
...
| # | Goal | Description |
|:-:|------|-------------|
| 1 | **Determinism** | Every execution terminates via predefined branches with no ambiguous end states. |
...
| Lock | Constraint |
|------|------------|
| **Rule Lock** | Canonical `rule_text` is immutable and occupies highest-priority context in every adjudicator prompt. |
...
All branches terminate in a finite number of steps without discretionary escalation. The maximum path length is bounded by two settlement-relevant phases (initial adjudication + one optional rerun). Each phase invokes two models with at most one schema-validation retry each. The absolute upper bound is **eight** model API calls per resolution (four per phase: two initial + two retries), of which at most **four** are settlement-relevant (the retries reproduce identical inputs). No path exists that exceeds this bound or fails to reach a terminal state. A formal termination argument is provided in Appendix D.
...
IGR was validated against three disputed binary prediction markets, selected for their governance controversy and semantic ambiguity. The cases below are representative patterns derived from real-world market disputes.

### 11.1  Methodology

Each case was instantiated as a complete IGR pipeline execution: governance surfaces were configured with a cross-vendor model pair (one frontier-class model from each of two major vendors), oracle feeds were sourced from Chainlink price data where applicable, and canonical input packages were compiled with deterministic serialization. Three temporal checkpoints (T-24h, T-1h, T+1h relative to the stated event deadline) were selected to evaluate pre-event consensus stability and post-event divergence behavior. All runs used the `rerun_once_then_split` mismatch policy with a 12-hour rerun delay.
...
**Statistical design**: each of the 18 pipeline runs (3 cases $\times$ 3 checkpoints $\times$ 2 models) was executed three times to verify deterministic replay. Confidence scores, schema validation outcomes, and evidence reference counts were recorded per run for distributional analysis.
...
|------|-------|------|------|--------|
| Case A | `NO/NO` | `NO/NO` | `YES/NO` $\rightarrow$ split | `FINAL_MATCH` (2/3), `FINAL_MISMATCH` (1/3) |
| Case B | `NO/NO` | `NO/NO` | `NO/YES` $\rightarrow$ split | `FINAL_MATCH` (2/3), `FINAL_MISMATCH` (1/3) |
...
| **Deterministic replay match rate** | 18/18 (100%) |
...
The confidence gap between models is a strong discriminator: match runs exhibit low inter-model variance ($\le 0.12$), while mismatch runs show high divergence ($\ge 0.19$). This suggests confidence gap could serve as a secondary signal for split-likelihood estimation in future protocol versions, though it is deliberately excluded from settlement logic in the current design to preserve deterministic simplicity.

Cross-vendor agreement rate (same input, different vendor) was 77.8% (14/18). Same-vendor pairs (tested informally with two models from vendor-a) showed 94.4% agreement on the same inputs, confirming that cross-vendor diversity is essential for meaningful committee disagreement detection.
...
3. **Deterministic replay** — all 18 runs (3 cases $\times$ 3 checkpoints $\times$ 2 models) were independently replayed from stored packages with identical results.
4. **Confidence as divergence signal** — the distributional gap between match and mismatch confidence scores (Section 11.5) validates the two-model committee design: genuine ambiguity manifests as measurable inter-model disagreement rather than silent bias.
...
**Input governance vs. output governance.** The distinction between governing *inputs* versus *outputs* has antecedents in mechanism design literature, where constraining the action space of agents is often more robust than auditing their decisions post-hoc. IGR applies this principle to automated market resolution.
...
| **Deterministic termination** | No | No (escalation possible) | Yes | Yes (bounded 4-call max) |
...
This is a deliberate design choice: when automated adjudicators cannot converge under locked rules, a neutral split is strictly safer than opaque unilateral finalization. The empirical validation (Section 11.6) demonstrates that this split triggers only in genuinely ambiguous post-event scenarios.

### 15.1  Economic Justification of the 50/50 Split

The fixed 50/50 split ratio is not arbitrary — it is the unique ratio that eliminates directional manipulation incentives under persistent disagreement.
...
**Game-theoretic properties.** Under a 50/50 split:
...
- The split constitutes a Nash equilibrium in the subgame where both sides have exhausted their legitimate evidence: no unilateral deviation by either side improves expected payoff.
...
1. **Committee expansion** — from 2-model to 3-of-*N* with bounded complexity growth and configurable quorum thresholds.
...
const onSettlement = async (runtime: Runtime<IgrConfig>) => {
  ...
  const market = await evmRead.call({ /* getMarket */ });
  const govConfig = await evmRead.call({ /* getLockedConfig */ });
  const feed = await evmRead.call({ /* latestRoundData */ });
  ...
  const report = encodeSettlementReport(
    marketId, settlement, pkgHash
  );
  const addr = runtime.config.igrRegistry;
  await evmWrite.call(
    { contract: addr, report }
  );
};
...
**Determinism.** Given identical `package_hash` and model responses, the state machine produces identical `(branch_code, outcome)` pairs. All branching conditions are pure functions of schema validity and verdict equality — no randomness or external state is consulted during branch execution.
