...
| CRE workflow pipeline scaffold (Log/Cron Trigger -> EVM Read -> HTTP x2 -> EVM Write) | `cre/igr-settlement/main.ts`, `cre/igr-settlement/workflow.yaml` | `bash cre/run-sim.sh` (local simulation-safe path) | `simulation/output/cre-sim/simulate.log`, `decision.json`, `hashes.txt` |
