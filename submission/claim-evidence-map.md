...
| CRE workflow pipeline scaffold (Log/Cron Trigger -> EVM Read -> HTTP x2 -> EVM Write) | `cre/igr-settlement/main.ts`, `cre/igr-settlement/workflow.yaml` | `cd cre && ~/.cre/bin/cre workflow simulate ./igr-settlement -T simulation-settings --non-interactive --trigger-index 0` | CLI simulation success output + `simulation/output/cre-sim/*` (if generated in local run) |
